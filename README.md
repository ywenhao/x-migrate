...
3. Choose follows, bookmarks, or both, then scan. A failed scan cannot start a transfer. Check the counts and preview, especially if X returns an uncertain end of list. After scanning, the two refresh buttons reread follows or bookmarks on both accounts without replacing the other list.
