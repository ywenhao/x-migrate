...
1. 分别登录旧账号和新账号。在各自浏览器会话的开发者工具中，从 Network 里选一个带 Cookie 的 X 请求并“复制为 curl”，粘贴到对应账号的输入框，点击“解析 curl”。也可以打开“Application / 应用 → Cookies → https://x.com”，手动复制 `auth_token` 和 `ct0` 的**值**。两个账号可使用不同的浏览器配置文件。
2. 在工具中分别连接旧账号和新账号。curl 文本在解析成功后清空；连接成功后，Cookie 值只保存在本地 Node.js 进程内，页面仅持有随机会话 ID；它们都不会写入磁盘，输入框在连接成功后清空。
...
5. 任务可停止。已执行的 X 操作不会自动回滚。若遇到限流或部分失败，稍后重新扫描即可跳过目标账号已经拥有的内容。
